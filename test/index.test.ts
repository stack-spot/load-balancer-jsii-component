...
import { LoadBalancerEnvComponent } from '../lib/index';

describe('LoadBalancerEnvComponent', () => {
  test('has loadBalancer property', () => {
    const app = new App();
    ...
    const environment = new LoadBalancerEnvComponent(stack, 'TestConstruct', { vpc, subnets });

    expect(environment.loadBalancer).toBeInstanceOf(ApplicationLoadBalancer);
  });
  ...
  test('has securityGroup property', () => {
    ...
    expect(environment.securityGroup).toBeInstanceOf(SecurityGroup);
  });
  test('creates load balancer', () => {
    ...
    new LoadBalancerEnvComponent(stack, 'TestConstruct', { vpc, subnets });
  ...
  test('has the option to enable internet facing on load balancer', () => {
    ...
    new LoadBalancerEnvComponent(stack, 'TestConstruct', { internetFacing: true, vpc, subnets });
    const template = Template.fromStack(stack);
  ...
  test('has the option to disable internet facing on load balancer', () => {
    ...
    new LoadBalancerEnvComponent(stack, 'TestConstruct', {
  ...
  test('reuses the VPC in the security group', () => {
    ...
    const securityGroupVpcId = securityGroup[securityGroupName]
      .Properties
      .VpcId
      .Ref;
  ...
  test('adds the security group to the load balancer', () => {
    ...
    const loadBalancer = template
      .findResources('AWS::ElasticLoadBalancingV2::LoadBalancer');
    const loadBalancerName = Object.keys(loadBalancer)[0];
    const loadBalancerSecurityGroupId = loadBalancer[loadBalancerName]
      .Properties
      .SecurityGroups[0]['Fn::GetAtt'][0];
  ...
  test('uses public subnets in the load balancer', () => {
    ...
    const loadBalancer = template
      .findResources('AWS::ElasticLoadBalancingV2::LoadBalancer');
    const loadBalancerName = Object.keys(loadBalancer)[0];
    const loadBalancerSubnetsRef = loadBalancer[loadBalancerName]
      .Properties
      .Subnets;
